...
**Location:** `/Users/fernando_idwell/Projects/Loci/go-templui/app/pkg/domain/chat_prompt/chat_service.go`
...
func (l *ServiceImpl) sendEvent(
    ...
    for i := 0; i < retries; i++ {
        ...
        case <-ctx.Done():
            ...
            l.logger.WarnContext(ctx, "Context cancelled, not sending stream event")
        ...
        default:
            ...
            case <-time.After(2 * time.Second):  // Timeout on slow consumer
                l.logger.WarnContext(ctx, "Dropped stream event due to slow consumer")
                l.deadLetterCh <- event
...
| Component | File Path |
|-----------|-----------|
| Service Implementation | `/app/pkg/domain/chat_prompt/chat_service.go` |
...
| Repository | `/app/pkg/domain/chat_prompt/chat_repository.go` |
| Tests | `/app/pkg/domain/chat_prompt/chat_service_test.go` |
