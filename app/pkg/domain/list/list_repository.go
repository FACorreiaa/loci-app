...
func (r *RepositoryImpl) GetListItem(ctx context.Context, listID, itemID uuid.UUID, contentType string) (models.ListItem, error) {
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return models.ListItem{}, fmt.Errorf("list item not found: %w", err)
		}
		r.logger.ErrorContext(ctx, "Failed to get list item", slog.Any("error", err))
		return models.ListItem{}, fmt.Errorf("failed to get list item: %w", err)
	}
	if dayNumber.Valid {
		dn := int(dayNumber.Int32)
		item.DayNumber = &dn
...
func (r *RepositoryImpl) GetListItemByID(ctx context.Context, listID, itemID uuid.UUID) (models.ListItem, error) {
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return models.ListItem{}, fmt.Errorf("no list item found for list_id %s and item_id %s", listID, itemID)
		}
