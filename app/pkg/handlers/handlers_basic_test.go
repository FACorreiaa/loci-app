...
func TestMain(m *testing.M) {
	...
	err := logger.Init(zapcore.InfoLevel, zap.String("service", "handlers-test"))
	if err != nil {
...
func TestUnifiedDataSourceConcept(t *testing.T) {
	t.Run("should demonstrate unified data source filtering", func(t *testing.T) {
		...
		activities := filterPOIsForActivities(allPOIs)
		...
		// Hotels filtering
		hotels := filterPOIsForHotels(allPOIs)
	...
	t.Run("should work with cache data structure", func(t *testing.T) {
		...
		// Retrieve and verify (this is what handlers would do)
		...
		activities := filterPOIsForActivities(cachedData.PointsOfInterest)
		hotels := filterPOIsForHotels(cachedData.PointsOfInterest)
