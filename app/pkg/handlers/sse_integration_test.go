...
var testChatService llmchat.LlmInteractiontService
var testChatRepo llmchat.Repository
var testProfileService profiles.Service
...
func TestSSE_EndToEndDataFlow(t *testing.T) {
	...
	t.Run("should stream itinerary data and filter for domain-specific handlers", func(t *testing.T) {
		// Setup router
		r := gin.Default()
...
func TestSSE_CacheIntegration(t *testing.T) {
	...
	t.Run("should populate cache during SSE stream and be accessible by domain handlers", func(t *testing.T) {
		// Setup
		r := gin.Default()
		r.Use(func(c *gin.Context) {
	...
	t.Run("should handle concurrent SSE streams and domain handler access", func(t *testing.T) {
		...
		for receivedSessions < numStreams {
			...
			case sessionID := <-sessionIDs:
				...
				// Test concurrent access to domain handlers
				go func(sid string) {
