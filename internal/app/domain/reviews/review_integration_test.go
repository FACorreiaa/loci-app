...
func TestMain(m *testing.M) {
	if err := godotenv.Load("../../../.env.test"); err != nil {
		log.Println("Warning: .env.test file not found for reviews integration tests.")
	}

	dbURL := os.Getenv("TEST_DATABASE_URL")
	if dbURL == "" {
		log.Fatal("TEST_DATABASE_URL environment variable is not set for reviews integration tests")
	}
	...
	if err != nil {
		log.Fatalf("Unable to create connection pool for reviews tests: %v\n", err)
	}
	defer testReviewDB.Close()

	if err := testReviewDB.Ping(context.Background()); err != nil {
		log.Fatalf("Unable to ping test database for reviews tests: %v\n", err)
	}
...
func TestReviewModels_Integration(t *testing.T) {
	...
	t.Run("Create and save reviews", func(t *testing.T) {
	...
	t.Run("Create and save reviews helpful", func(t *testing.T) {
		// Create a reviews first
		...
		// Verify reviews helpful was saved
	...
	t.Run("Create and save reviews reply", func(t *testing.T) {
		// Create a reviews first
		...
		// Verify reviews reply was saved
