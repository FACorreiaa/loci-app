...
func insertTestPOI(t *testing.T, id uuid.UUID, name string, cityID uuid.UUID) models.POIDetail {
	t.Helper()
	poi := models.POIDetail{ID: id, Name: name, CityID: cityID, Latitude: 1.0, Longitude: 1.0, Category: "Test"} // Add other required fields
...
func TestPOIServiceImpl_SearchPOIs_Integration(t *testing.T) {
	...
	poiMuseum := models.POIDetail{ID: uuid.New(), CityID: searchCityID, Name: "Grand Museum", Category: "Museum", Latitude: 10.0, Longitude: 10.0, DescriptionPOI: sql.NullString{String: "Ancient artifacts", Valid: true}}
	poiPark := models.POIDetail{ID: uuid.New(), CityID: searchCityID, Name: "City Park", Category: "Park", Latitude: 10.1, Longitude: 10.1, DescriptionPOI: sql.NullString{String: "Green space", Valid: true}}
	poiCafe := models.POIDetail{ID: uuid.New(), CityID: searchCityID, Name: "Art Cafe", Category: "Cafe", Latitude: 10.05, Longitude: 10.05, DescriptionPOI: sql.NullString{String: "Coffee and art", Valid: true}, Tags: []string{"art", "coffee"}}
	...
	t.Run("Search by category", func(t *testing.T) {
		filter := models.POIFilter{CityID: searchCityID, Category: "Museum"}
	...
	t.Run("Search by tag", func(t *testing.T) {
		filter := models.POIFilter{CityID: searchCityID, Tags: []string{"art"}}
		pois, err := testService.SearchPOIs(ctx, filter)
		require.NoError(t, err)
	...
	t.Run("Search by name substring", func(t *testing.T) {
		filter := models.POIFilter{CityID: searchCityID, Name: "grand"} // Case-insensitive search usually
		pois, err := testService.SearchPOIs(ctx, filter)
		require.NoError(t, err)
		require.Len(t, pois, 1)
		assert.Equal(t, "Grand Museum", pois[0].Name)
	...
	t.Run("Search with no results", func(t *testing.T) {
		filter := models.POIFilter{CityID: searchCityID, Category: "Zoo"}
		pois, err := testService.SearchPOIs(ctx, filter)
		require.NoError(t, err)
		assert.Empty(t, pois)
	})
...
func insertPOIForIntegration(t *testing.T, poi models.POIDetail) {
	t.Helper()
	_, err := testDB.Exec(context.Background(),
...
// TEST_DATABASE_URL="postgres://user:password@localhost:5432/test_db_name?sslmode=disable" go test -v ./app/api/poi -tags=integration -count=1
