...
func TestUnifiedDataSource_Integration(t *testing.T) {
	...
	t.Run("should use unified data source across all domain common", func(t *testing.T) {
		...
		t.Run("activities handler should filter activities from unified data", func(t *testing.T) {
			...
			filteredActivities := activities.filterPOIsForActivities(testPOIs)
		...
		t.Run("hotels handler should filter hotels from unified data", func(t *testing.T) {
			...
			filteredHotels := hotels.filterPOIsForHotels(testPOIs)
...
func TestUnifiedDataSource_CacheConsistency(t *testing.T) {
	...
	t.Run("should return consistent data across all domain common", func(t *testing.T) {
		...
		for _, h := range handlers {
			t.Run("consistency for "+h.name, func(t *testing.T) {
				...
				case *activities.ActivitiesHandlers:
					component = handler.HandleActivitiesPage(c)
				case *hotels.HotelsHandlers:
					component = handler.HandleHotelsPage(c)
				case *restaurants.RestaurantsHandlers:
		...
		activities := activities.filterPOIsForActivities(testCompleteData.PointsOfInterest)
		hotels := hotels.filterPOIsForHotels(testCompleteData.PointsOfInterest)
