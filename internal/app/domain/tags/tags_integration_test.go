...
func TesttagsServiceImpl_Integration(t *testing.T) {
	...
	t.Run("CreateTag for user1", func(t *testing.T) {
		...
		params2 := models.CreatePersonalTagParams{Name: "Quiet Study"}
	...
	t.Run("GetTag for user1 - tag belonging to user2 (should fail or return not found)", func(t *testing.T) {
		...
		// Assert specific error if your repo/service returns one (e.g., models.ErrNotFound)
	...
	t.Run("Update tag for user1", func(t *testing.T) {
		...
		updateParams := models.UpdatePersonalTagParams{
			Name:        &newName,
			Description: &newDesc,
		}
	...
	t.Run("Update tag not belonging to user (should fail)", func(t *testing.T) {
		...
		updateParams := models.UpdatePersonalTagParams{Name: &newName}
...
// TEST_DATABASE_URL="postgres://user:password@localhost:5432/test_db_name?sslmode=disable" go test -v ./app/tags -tags=integration -count=1
