...
func NewHotelFavoritesHandlers(poiService poi.Service, logger *zap.Logger) *HotelFavoritesHandlers {
	return &HotelFavoritesHandlers{
		poiService: poiService,
		logger:     logger,
	}
}

func (h *HotelFavoritesHandlers) AddHotelFavorite(c *gin.Context) {
	id := c.Param("id")
	...
	if err != nil {
		h.logger.Error("Invalid hotel ID", zap.String("hotel_id", id), zap.Error(err))
		c.HTML(http.StatusBadRequest, "", `<div class="text-red-500 text-sm">Invalid hotel ID</div>`)
		return
	}

	h.logger.Info("Adding hotel to favorites",
		zap.String("hotel_id", id),
		zap.String("user_id", userIDStr),
	)

	// Add to database
	_, err = h.poiService.AddHotelToFavourites(c.Request.Context(), userID, hotelID)
	if err != nil {
		h.logger.Error("Failed to add hotel to favorites", zap.Error(err))
	...
	h.logger.Info("Successfully added hotel to favorites",
		zap.String("hotel_id", id),
		zap.String("user_id", userIDStr),
	)
}

func (h *HotelFavoritesHandlers) RemoveHotelFavorite(c *gin.Context) {
	id := c.Param("id")
	userIDStr := middleware.GetUserIDFromContext(c)
	...
	h.logger.Info("Removing hotel from favorites",
		zap.String("hotel_id", id),
		zap.String("user_id", userIDStr),
	)

	// Remove from database
	err = h.poiService.RemoveHotelFromFavourites(c.Request.Context(), userID, hotelID)
	if err != nil {
		h.logger.Error("Failed to remove hotel from favorites", zap.Error(err))
	...
	h.logger.Info("Successfully removed hotel from favorites",
		zap.String("hotel_id", id),
		zap.String("user_id", userIDStr),
	)
}
