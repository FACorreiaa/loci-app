...
func createTestUserDirectly(t *testing.T, user models.User) uuid.UUID {
	t.Helper()
...
func TestServiceUserImpl_UserProfile_Integration(t *testing.T) {
	...
	testUser := models.User{ // Assuming models.User is the struct for your DB table
	...
	t.Run("Update user profile", func(t *testing.T) {
		dob := "1990-01-01"
		updateParams := models.UpdateProfileParams{
...
func TestServiceUserImpl_UserStatus_Integration(t *testing.T) {
	...
	testUser := models.User{Username: "status_user", Email: "status@example.com", PasswordHash: "hash"}
...
// TEST_DATABASE_URL="postgres://user:password@localhost:5432/test_db_name?sslmode=disable" go test -v ./app/user -tags=integration -count=1
